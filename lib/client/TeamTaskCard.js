...
export function TeamTaskCard({ node }) {
    ...
    return (_jsxs("section", { className: css.root, "data-team-task-card": true, "data-task-id": data.taskId, children: [_jsxs("header", { className: css.head, children: [_jsx("span", { className: css.mark, children: _jsx(TeamMark, { size: 14 }) }), _jsx("span", { className: css.name, children: live?.state.name ?? data.taskName }), _jsx("span", { className: css.tag, children: "team-task" })] }), (live?.state.goal ?? data.goal) !== '' && _jsx("p", { className: css.goal, children: live?.state.goal ?? data.goal }), _jsxs("footer", { className: css.foot, children: [_jsx("span", { className: css.meta, children: finished !== undefined
                            ? `finished: ${finished}`
                            : progress === undefined
                                ? `${data.seededNodes} nodes planned`
                                : `${progress.done}/${progress.total} approved · ${members ?? 0} members` }), _jsx("button", { type: "button", className: css.boardButton, onClick: () => {
