...
export type NodeStatus = 'pending' | 'dispatched' | 'running' | 'awaiting_review' | 'approved' | 'cancelled';
/** Statuses that unblock nothing and accept no further runs. */
export declare const TERMINAL_NODE_STATUSES: readonly NodeStatus[];
...
export declare const LEAD_KEY = "lead";
...
export type TeamTaskEvent = {
    ...
    type: 'node_updated';
    key: string;
    patch: Partial<Omit<NodeSpec, 'key'>>;
} | {
    type: 'node_cancelled';
    key: string;
} | {
    ...
    type: 'message_sent';
    message: Omit<Message, 'deliveredAt'>;
} | {
...
export type LogLine = TeamTaskEvent & {
    seq: number;
    ts: number;
};
