...
import { listTaskIds, readState } from "./log.js";
/**
...
export function registerTeamTaskCommand(ctx, stateDir) {
        ...
        handler: async (invocation) => {
            ...
            if (argument === '' || argument === 'status') {
                ...
                for (const taskId of await listTaskIds(stateRoot)) {
                    ...
                    if (state === undefined || state.leadSessionId !== agent.id)
                        continue;
                    ...
                    lines.push(`${state.name} — ${approved}/${active.length} approved`
