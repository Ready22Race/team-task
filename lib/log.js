...
export function parseRawLog(raw) {
    ...
    for (let index = 0; index < segments.length; index += 1) {
        ...
        try {
            ...
            if (isLastContent)
                return { lines, goodByteLength: good, tornTail: true };
...
export async function mutateTask(stateRoot, taskId, propose) {
    return withTaskLock(`${stateRoot} ${taskId}`, async () => {
        ...
        if (parsed.corruptLine !== undefined) {
            throw new Error(`event log corrupted at line ${parsed.corruptLine} (${logFile}) — `
...
export function validatePlanGraph(state) {
    ...
    for (const node of state.nodes) {
        ...
        for (const dep of node.dependsOn) {
            if (dep === node.key)
                return `node "${node.key}" depends on itself`;
            if (!keys.has(dep))
                return `node "${node.key}" depends on unknown node "${dep}"`;
        }
    }
    ...
    const visit = (key) => {
        ...
        for (const dep of nodesByKey.get(key)?.dependsOn ?? []) {
            ...
            if (mark === 1)
                return `dependency cycle through "${dep}"`;
            if (mark === undefined) {
    ...
    for (const node of state.nodes) {
        if (node.status === 'cancelled' || color.has(node.key))
            continue;
...
export function hasCurrentClaim(node) {
