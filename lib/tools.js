...
import { createMessage, identityOf, listTaskIds, mintTaskId, mutateTask, readState, undeliveredTo, unsatisfiedDependencies, } from "./log.js";
import { deliverToMember, interruptMember, memberActivity, steerLead, } from "./members.js";
import { LEAD_KEY, } from "./types.js";
/** The caller agent, or a loud failure for non-agent callers. */
function requireAgent(exec) {
    if (!exec.agent)
        throw new Error('team_task tools require a calling agent');
    return exec.agent;
}
...
async function findParticipantTask(stateRoot, callerId) {
    for (const taskId of await listTaskIds(stateRoot)) {
        ...
        if (state === undefined || state.finishedAt !== undefined)
            continue;
        if (identityOf(state, callerId) !== undefined)
            return state;
    }
    return undefined;
}
/** The most recently finished task this caller participated in, if any. */
async function findFinishedTask(stateRoot, callerId) {
    let latest;
    for (const taskId of await listTaskIds(stateRoot)) {
        const state = await readState(stateRoot, taskId);
        if (state === undefined || state.finishedAt === undefined)
        ...
        if (identityOf(state, callerId) === undefined)
            continue;
        if (latest === undefined || (state.finishedAt ?? 0) > (latest.finishedAt ?? 0))
            latest = state;
    }
    return latest;
}
async function requireTask(stateRoot, callerId) {
    ...
    if (finished !== undefined) {
        throw new Error(`your previous team task "${finished.name}" is finished (${finished.finishStatus ?? 'completed'}) and archived — `
...
function requireLead(state, callerId) {
...
function nodePatchFromArgs(raw) {
    return {
        key: raw.key,
...
function statusView(ctx, state, callerId) {
...
export function registerTeamTaskTools(ctx, config, scheduler) {
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            ...
            if (existing !== undefined) {
                ...
                throw new Error(`this session already leads active task "${existing.name}" (${open} node(s) not yet approved). `
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args) {
            if (!PLAYBOOKS.has(args.role))
                throw new Error(`unknown playbook "${args.role}"`);
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            ...
            requireLead(state, lead.id);
            const events = [
                ...(args.add ?? []).map(raw => ({ type: 'node_planned', node: nodeSpecFromArgs(raw) })),
                ...(args.update ?? []).map((raw) => {
                    const { key, ...patchRaw } = nodePatchFromArgs(raw);
                    return { type: 'node_updated', key, patch: patchRaw };
                }),
                ...(args.cancel ?? []).map(key => ({ type: 'node_cancelled', key })),
            ];
            if (events.length === 0)
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            ...
            if (node === undefined)
                throw new Error(`no node "${args.key}"`);
            ...
            if (node.status === 'dispatched' || node.status === 'running') {
                ...
                throw new Error(`node "${args.key}" is already ${node.status} with ${node.assignee} — `
                    + 'reassignment is not supported yet; rework after settle instead');
            }
            const blocked = unsatisfiedDependencies(state, node);
            if (blocked.length > 0)
                throw new Error(`node "${args.key}" waits on approval of: ${blocked.join(', ')}`);
            ...
            if (member === undefined)
                throw new Error(`no member "${args.assignee}"`);
            const { state: next } = await mutateTask(stateRoot, state.id, fresh => [{
        ...
        async execute(args, exec) {
            ...
            for (;;) {
                if (exec.signal.aborted)
                    throw exec.signal.reason ?? new Error('await cancelled');
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            ...
            if (node === undefined)
                throw new Error(`no node "${args.node_key}"`);
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            ...
            const lead = state.leadSessionId === caller.id
                ? caller
                : ctx.agents.get(state.leadSessionId);
            if (to === LEAD_KEY) {
                if (lead !== undefined && from !== LEAD_KEY)
                    delivered = steerLead(lead, from, args.content);
            }
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(_args, exec) {
            ...
            return statusView(ctx, fresh, caller.id);
        },
    }));
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            ...
            for (const member of state.members) {
                if (member.retired !== true && member.sessionId !== '')
                    interruptMember(ctx, lead, member.sessionId);
            }
            await mutateTask(stateRoot, state.id, () => [
                ...state.members.filter(m => m.retired !== true)
                    .map(m => ({ type: 'member_retired', name: m.name })),
                {
                    type: 'task_finished',
                    status: args.status,
